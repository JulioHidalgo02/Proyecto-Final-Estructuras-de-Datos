...
public class SodaFide {
    ...
    private NodoListaSC adelante;
    private NodoListaSC atras;
    ...
    public void LlenarLSC() {
        Trabajadores t = new Trabajadores();
        t.setPuesto(JOptionPane.showInputDialog("Digite su puesto laboral"));
        t.setHorario(JOptionPane.showInputDialog("Digite su horario"));
        NodoListaSC nuevo = new NodoListaSC();
        nuevo.setDato(t);
        if (esVaciaLSC()) {
            adelante = nuevo;
            atras = nuevo;
            atras.setSiguiente(adelante);
        } else if (t.getPuesto().compareTo(adelante.getDato().getPuesto()) < 0) {
            nuevo.setSiguiente(adelante);
            adelante = nuevo;
            atras.setSiguiente(adelante);
        } else if (t.getPuesto().compareTo(atras.getDato().getPuesto()) >= 0) {
            atras.setSiguiente(nuevo);
            atras = nuevo;   //fin=fin.getSiguiente();
            atras.setSiguiente(adelante);
        } else {
            NodoListaSC aux = adelante;
            while (aux.getSiguiente().getDato().getHorario().compareTo(t.getHorario()) < 0) {
    ...
    public void mostrarElementosLSC() {
        String s = "";
        if (!esVaciaLSC()) {
            NodoListaSC aux = adelante;
            s = s + "Puesto: " + aux.getDato().getPuesto() + " Horario: " + aux.getDato().getHorario() + " <-- ";
            aux = aux.getSiguiente();
            while (aux != adelante) {
                s = s + "Puesto: " + aux.getDato().getPuesto() + " Horario: " + aux.getDato().getHorario() + " <-- ";
                aux = aux.getSiguiente();
            ...
            JOptionPane.showMessageDialog(null,
                    "Los horarios de los trabajadores del día son:\n" + s);
        } else {
            JOptionPane.showMessageDialog(null,
                    "No se puede mostrar, lista vacía!");
        }
